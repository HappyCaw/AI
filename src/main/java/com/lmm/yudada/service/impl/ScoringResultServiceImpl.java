...
import com.lmm.yudada.mapper.ScoringResultMapper;
...
public class ScoringResultServiceImpl extends ServiceImpl<ScoringResultMapper, ScoringResult> implements ScoringResultService {
    ...
    public void validScoringResult(ScoringResult scoringResult, boolean add) {
        ...
        if (add) {
            // todo 补充校验规则
            ThrowUtils.throwIf(StringUtils.isBlank(resultName), ErrorCode.PARAMS_ERROR,"得分结果名称不能为空");
        ...
        if (appId != null){ //无论是新增验证，还是更新验证，都需要校验输入的appId是否合法
            App app = appService.getById(appId);
            ThrowUtils.throwIf(app == null, ErrorCode.PARAMS_ERROR,"应用非法");
        }
    }
